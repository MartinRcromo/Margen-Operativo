    ...
    return (
        <tr className={product.Activo ? "" : "off"}>
            <td>
                <label className="toggle">
                    <input
                        ...
                        aria-label={`${product.Activo ? 'Desactivar' : 'Activar'} ${product.Producto}`}
                    />
                    <span className={`pill ${product.Activo ? "good" : "bad"}`}>
                        <span className="dot"></span>{product.Activo ? "Activo" : "Inactivo"}
                    </span>
                </label>
            </td>
            ...
            <td style={{ textAlign: 'center', fontWeight: 700 }}>
                {product.Producto}
                {product._sim && <span className="pill warn" style={{marginLeft:'6px', fontSize:'10px', padding: '2px 6px'}}>SIM</span>}
            </td>
            ...
            <td style={{ fontWeight: 600, textAlign: 'center' }}>{product.MarkUp === null ? "-" : pct2((product.MarkUp || 0) * 100)}</td>
            <td style={{ fontWeight: 600, textAlign: 'center' }}>{money(product.Resultado)}</td>

            <td style={{ fontWeight: 700, color: 'var(--accent)', textAlign: 'center' }}>{money(product.MargenOper)}</td>
            <td style={{ fontWeight: 700, textAlign: 'center' }}>{pct2((product.MargenPct || 0) * 100)}</td>
            <td style={{ textAlign: 'center' }}>{product.ROI === null ? "-" : pct2(product.ROI * 100)}</td>
            <td style={{ textAlign: 'center' }}>
                <div style={{ display: 'flex', gap: '4px', justifyContent: 'center' }}>
                    <button className="btn" onClick={() => onSelectProduct(product)} style={{ padding: '6px 10px' }} title="Configurar escenario individual para este producto">
                        Escenario
                    </button>
                </div>
            </td>
